...
sub vcl_recv {
	...
	set req.http.X-Forwarded-Proto = "https"; # Comment if not using SSL
	...
	# pass wp-admin urls
	if (req.url ~ "^/login" || req.url ~ "^/cms/(wp-login.php|wp-admin)" || req.url ~ "preview=true" || req.url ~ "^/cms/xmlrpc.php") {
		return (pass);
	}
